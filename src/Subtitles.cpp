...
void Subtitles::render() {
  ...
  int textWidth = Settings::viewport.first - 2 * margin.first;

  std::pair<float, float> textSize = Text::instance().getTextSize(subtitle,
    {textWidth, fontHeight},
    0);
    textSize.first *= Settings::viewport.first / 2.0f;
    textSize.second *= Settings::viewport.second / 2.0f;

    Text::instance().render(subtitle,
    {(Settings::viewport.first - textSize.first) / 2, margin.second + textSize.second - fontHeight},
    {textWidth, fontHeight},
    0,
    {1.0, 1.0, 1.0, 1.0},
    true);
